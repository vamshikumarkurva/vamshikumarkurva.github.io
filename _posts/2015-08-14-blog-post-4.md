...
1. Two eigen vectors corresponding to standard basis in $$\mathbb{R}^2$$. Vertical one stretched twice its size($$\lambda_1 = 2.0$$) and the horizontal one made half it's size ($$\lambda_2 = 0.2$$). So $det = 1$, the mapping is area preserving.
...
5. Two eigen values $$\lambda_1 = 0.5, \lambda_2 = 1.5$$. Scales the space along one vector by a factor of 0.5 and along the other by a factor of 1.5. Since $$det = 0.75$$, mapping scales the space by 75%
