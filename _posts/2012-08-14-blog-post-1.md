...
2. For identical distributions $$q$$ and $$p$$,  $$KL(q || p) = 0$$

3. $$KL (q || p) \geq 0$$
*proof*: 
\\[
\begin{aligned}
    -KL(q || p) &= \math{E}_{q} (\log(-\frac{q}{p}) = \math{E}_{q} (\log(\frac{p}{q}) \newline
    &\leq \log(\math{E}_{q} \frac{p}{q}) = \log(1) = 0 
\end{aligned}
\\]
