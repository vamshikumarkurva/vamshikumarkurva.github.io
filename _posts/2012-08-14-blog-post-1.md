...
------
 ...
 <p align="center">
  <img src="https://github.com/vamshikumarkurva/vamshikumarkurva.github.io/blob/master/_posts/Images/KL-example.png?raw=true" alt="Photo" style="width: 450px;"/>
</p>
![](Images/KL-example.png)
...
1. KL divergence measure is not symmetric, and hence not a metric $$KL(q || P) \neq KL(p || q)$$.

2. For identical distributions $$KL(q || p) = 0$$

3. $KL (q || p) \geq 0$
*proof*: 
\\[
\begin{aligned}
    -KL(q || p) &= \mathbf{E}_{q} (\log(-\frac{q}{p}) = \mathbf{E}_{q} (\log(\frac{p}{q}) \newline
    &\leq \log(\mathbf{E}_{q} \frac{p}{q}) = \log(1) = 0 
\end{aligned}
\\]
