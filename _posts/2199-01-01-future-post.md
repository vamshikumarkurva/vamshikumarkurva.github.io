...
Set is a collection of some elements and space is the collection of all elements under consideration. For example, $$A_1=\{1\}, A_2=\{1,5,10,12\}$$ are all sets contained in the space of natural numbers, $$\math{N}=\{0,1,…\}$$. A point set or atomic set is a set containing a single element such as $$\{1\}$$ above. The entire space itself is always a valid set, as is the empty set or null set, $$\emptyset$$, which contains no elements at all. Sets are often defined implicitly via an inclusion criterion. These sets are denoted with the set builder notation. For example,

$$
\math{R}^{+} = \{x \in \math{R}| x > 0\}
$$
...
*How to construct Borel field?* Take all possible open intervals.Take their compliments. Take arbitrary unions. Include $$\emptyset$$ and $$\math{R}$$. $$B_R$$ contains a wide range of intervals including open, closed, and half-open intervals. It also contains disjoint intervals such as $${(2, 7] \cup (19, 32)}$$. It contains (nearly) every possible collection of intervals that are imagined.
...
- $$0 \leq \mu(A) \leq \infty \forall A \in \Sigma$$
- $$\mu(\emptyset) = 0$$
...
Even though the blogpost is short, it took time for me to understand certain concepts and put together these things. The following sources were very helpful in understanding few concepts.

1. [Probability Theory (For Scientists and Engineers)](https://betanalpha.github.io/assets/case_studies/probability_theory.html)
2. [Why do we need sigma-algebras to define probability spaces?](https://stats.stackexchange.com/questions/199280/why-do-we-need-sigma-algebras-to-define-probability-spaces)
3. [Why do we need random variables?](https://mathoverflow.net/questions/250500/why-do-we-need-random-variables)
4. [Probability Theory: Introduction](https://www.bauer.uh.edu/rsusmel/phd/sR-0.pdf)
